        ...
        for (int i = 0; i < size; i++) {
            if (str.equalsIgnoreCase(headers.name(i))) {
                ...
                while (i2 < strValue.length()) {
                    int iSkipUntil = HeaderParser.skipUntil(strValue, i2, MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR);
                    String strTrim = strValue.substring(i2, iSkipUntil).trim();
                    int iSkipWhitespace = HeaderParser.skipWhitespace(strValue, iSkipUntil);
                    if (!strValue.regionMatches(true, iSkipWhitespace, "realm=\"", 0, "realm=\"".length())) {
                        break;
                    }
                    int length = iSkipWhitespace + "realm=\"".length();
